...
readonly DEFAULT_OPTIONS=("Sys-link" "Vim Tamplates" "Nvim Tamplates" ">>> MAINMENU <<<")
...
confirm() {
    if $will_return_index; then
        ...
        for index in ${!options[@]}; do
            if value_in_array "$index" "${selected_options[@]}"; then
                case $index in
			0) sudo -u batan bash dot.sh && sudo -u batan bash /home/batan/check/dotmenu.sh;;
			1) sudo -u batan bash vimtemplates.sh && sudo -u batan bash /home/batan/check/dotmenu.sh;;
			2) sudo -u batan bash nvimtemplates.sh &&sudo -u batan bash /home/batan/check/dotmenu.sh;;
			3) sudo -u batan bash first.sh ;;
