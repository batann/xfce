...
readonly DEFAULT_OPTIONS=(">>> BACK  UP <<< " ">>> DOTFILES <<< " ">>> SOFTWARE <<<" ">>> SETTINGS <<<" ">>> COMMANDS <<<" ">>> VIM CMD  <<<" ">>> Reminder <<<" ">>> ProjMent <<<")
...
confirm() {
    if $will_return_index; then
        ...
        for index in ${!options[@]}; do
            if value_in_array "$index" "${selected_options[@]}"; then
                case $index in
                    0) sudo -u batan bash /home/batan/check/backup.sh ;;
	     			1) sudo -u batan bash /home/batan/check/dotmenu.sh ;;
	       			2) sudo -u batan bash /home/batan/check/software.sh ;;
				    3) sudo -u batan bash /home/batan/check/settings.sh ;;
                	4) sudo -u batan bash /home/batan/check/commands.sh ;;
                	5) sudo -u batan bash /home/batan/check/vim.commands.sh ;;
					6) sudo -u batan bash /home/batan/check/reminder.sh ;;
					7) sudo -u batan bash /home/batan/check/PM.sh ;;
			   esac
            fi
        done
    fi
