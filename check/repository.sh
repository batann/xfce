...
readonly DEFAULT_OPTIONS=("Ranger" "Cmus" "Flatpak" "Git" "Phython-pip" "Taskwarrior" "Timewarrior" "Sweeper" "Chatgpt" "Exhuberant-ctags" "Vit" "Neovim" "Pandoc" "manhtml" "bleachbit" "openssh-server" "openssh-client" "renameutils" "mat" "ffmpeg" "buku" "megatools" "links" "trash-cli" "tmux" "yad" "nodau" "pwman" "bwmw-ng" "vnstat" "Turtle Note" "Festival" "Espeak" "Festvox-us-slt-hts" "fzf" "rofi" "ddgr" "tldr" "Newsboat" "Falkon Browser" "Kodi" "xterm" ">>> SOFTWARE <<<" "NODEJS 20"">>> MAINMENU <<<")
#===============================================================================
# VARIABLES
#===============================================================================
cursor=0
...
0) sudo pacman -Sy ranger -y ;;
1) sudo pacman -Sy cmus -y ;;
2) sudo pacman -Sy flatpak -y ;;
3) sudo pacman -Sy git -y ;;
4) sudo pacman -Sy phython-pip -y ;;
5) sudo pacman -Sy taskwarrior -y ;;
6) sudo pacman -Sy timewarrior -y ;;
7) sudo pacman -Sy sweeper -y ;;
8) sudo pacman -Sy chatgpt -y ;;
9) sudo pacman -Sy exhuberant-ctags -y ;;
10) sudo pacman -Sy vit -y ;;
11) sudo pacman -Sy neovim -y ;;
12) sudo pacman -Sy pandoc -y ;;
13) sudo pacman -Sy manhtml -y ;;
14) sudo pacman -Sy bleachbit -y ;;
15) sudo pacman -Sy openssh-server -y ;;
16) sudo pacman -Sy openssh-client -y ;;
17) sudo pacman -Sy renameutils -y ;;
18) sudo pacman -Sy mat -y ;;
19) sudo pacman -Sy ffmpeg -y ;;
20) sudo pacman -Sy buku -y ;;
21) sudo pacman -Sy megatools -y ;;
22) sudo pacman -Sy links -y ;;
23) sudo pacman -Sy trash-cli -y ;;
24) sudo pacman -Sy tmux -y ;;
25) sudo pacman -Sy yad -y ;;
26) sudo pacman -Sy nodau -y ;;
27) sudo pacman -Sy pwman -y ;;
28) sudo pacman -Sy bwmw-ng -y ;;
29) sudo pacman -Sy vnstat -y ;;
30) sudo pacman -Sy turtle note -y ;;
31) sudo pacman -Sy festival -y ;;
32) sudo pacman -Sy espeak -y ;;
33) sudo pacman -Sy festvox-us-slt-hts -y ;;
34) sudo pacman -Sy fzf -y ;;
35) sudo pacman -Sy rofi -y ;;
36) sudo pacman -Sy ddgr -y ;;
37) sudo pacman -Sy tldr -y ;;
38) sudo pacman -Sy newsboat -y ;;
39) sudo pacman -Sy falkon browser -y ;;
40) sudo pacman -Sy kodi -y ;;
41) sudo pacman -Sy xterm -y ;;
42) mkdir arena &&
	cd arena &&
	wget http://www.playwitharena.de/downloads/arenalinux_64bit_3.10beta.tar.gz &&
	tar vfxz arenalinux*.tar.gz &&

43) wget https://nodejs.org/dist/v20.12.2/node-v20.12.2-linux-x64.tar.xz
tar xz node-v20.12.2-linux-x64.tar.xz

44)
			sudo -u batan bash software.sh
			;;
		45)
			sudo -u batan bash first.sh
			;;
