...
readonly DEFAULT_OPTIONS=(">>> Apt Repo <<<" ">>> Vim      <<<" ">>> Neovim   <<<" ">>> Flatpaks <<<" ">>> Github   <<<" ">>> Progress <<<" ">>> MAINMENU <<<")
...
confirm() {
    if $will_return_index; then
        ...
        for index in ${!options[@]}; do
            if value_in_array "$index" "${selected_options[@]}"; then
                case $index in
                    0) echo "Executing script for Software"
                       sudo -u batan bash repository.sh
		       ;;
                    1) echo "Executing script for Vim & Vim Plugins"
                       sudo -u batan bash vim.sh
		       ;;
                    2) echo "Executing script for Neovim & Neovim Plugins"
                       sudo -u batan bash neovim.sh
		       ;;
                    3) echo "Executing script for Flatpaks"
                       sudo -u batan bash flatpaks.sh
		       ;;
                    4) echo "Executing script for Github"
                       sudo -u batan bash github.sh
		       ;;
                    5) echo "Executing script for In Progress"
                       sudo -u batan bash progress.sh
		       ;;

	       6) echo "Executing script first.sh"
