...
readonly DEFAULT_OPTIONS=("Firefox Developer" "Edge-tts"  ">>> SOFTWARE <<<" ">>> MAINMENU <<<")
...
confirm() {
    if $will_return_index; then
        ...
        for index in ${!options[@]}; do
            if value_in_array "$index" "${selected_options[@]}"; then
                case $index in
			0) wget https://download-installer.cdn.mozilla.net/pub/devedition/releases/124.0b4/linux-x86_64/en-US/firefox-124.0b4.tar.bz2
				tar xvjf firefox-124.0b4.tar.bz2
				;;
			1) sudo -u batan bash ;;
			2) sudo -u batan bash ;;
			3) sudo -u batan bash ;;
			...
			7) sudo -u batan bash ;;
			8) sudo -u batan bash ;;
			9) sudo -u batan bash software.sh ;;
			10) sudo -u batan bash first.sh ;;
